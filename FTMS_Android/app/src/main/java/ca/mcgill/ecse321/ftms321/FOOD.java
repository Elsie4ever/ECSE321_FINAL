...
public class FOOD extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TableLayout table;
        try {
            //call the showFood function in controller
            pc.showFood();
            //get the manager
            Manager m = Manager.getInstance();
            //get all the food from backend
            List<Food> foodlist = m.getFoods();
            //find the tablelayout in frontend in order to create things in it later
            table = (TableLayout) findViewById(R.id.table_foods);
            table.setColumnStretchable(0,true);
            table.setColumnStretchable(1,true);
            table.setColumnStretchable(2,true);
            //create name, quantity and price in each row, the number of row is same as quantity of food the backend has
...
//
    ...
    //onClick method: go to ADDFOOD page
    ...
    //onClick method: go to FOOD page
    public void deleteFood(Button button){
